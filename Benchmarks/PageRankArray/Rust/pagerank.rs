...
impl Graph {
    fn new(path: &str) -> Result<Graph, Box<dyn std::error::Error>> {
        ...
        let mut incoming: Vec<Vec<i32>> = Vec::new();
        let mut outgoing: Vec<Vec<i32>> = Vec::new();
        ...
        for line in reader.lines() {
            ...
            if line.starts_with('p') {
                ...
                outgoing.get_mut(v0 as usize).unwrap().push(v1);
                incoming.get_mut(v1 as usize).unwrap().push(v0);
            }
        }
